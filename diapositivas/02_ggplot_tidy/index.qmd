...
[^2]: El [capítulo 10](https://r4ds.had.co.nz/tibbles.html){target="_blank"} de la primera edición de R4DS o [https://tibble.tidyverse.org/](https://tibble.tidyverse.org/){target="_blank"} son buenas referencias para profundizar en el tema.
...
<br/> **Respuesta: **El mapeo estético está asociado a variables, no a descripciones!
...
- `filename` es el directorio y el nombre del archivo

- Soporta `.jpg`, `.png`, `.pdf` y otros formatos

- La otra manera es con el botón de exportar en el visualizador de gráficos
...
1. Usen `geom_bar()` como objeto geométrico y mapeen el eje `x` a `continent`.

. . . 

2. Los gráficos llenos de gris no son muy atractivos. Vamos a agregar color al gráfico mapeando el atributo `fill` a `continent`.

. . . 

3. Ahora que el gráfico tiene color, vamos a mejorar aún más su estética. Revisen la página de [paletas de colores Brewer](https://r-graph-gallery.com/38-rcolorbrewers-palettes.html){target="_blank"}, elijan una de las paletas de colores que allí encuentran y agreguen la capa `scale_fill_brewer(palette = "nombre_de_paleta")`. 


## 💪 Ejercicio 3 {.medium}

4. Para ser más claros, modifiquen los nombres de los ejes a `y = "Frequency"` y `x = "Continent"`. La diapositiva 41 tiene un ejemplo de cómo hacerlo.

. . .

5. En este caso la guía de los continentes parece innecesaria. Eliminémosla agregando la capa `guides(fill=FALSE)`

. . . 

6. `ggplot` usa un fondo gris por defecto. Elijan entre `theme_minimal()` o `theme_classic()` como última capa.


## 💪 Ejercicio 3 {.medium}

Deberían tener algo similar a esto:
...
## Los datos `starwars` {.medium}

Para ilustrar el uso del paquete `dplyr`, utilizaré  los datos `starwars`, que vienen incluidos en el mismo paquete:
...
2. Agreguen una columna que sea el logaritmo del GDP per cápita (Pista: usen la función `log()`)
