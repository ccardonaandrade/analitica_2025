...
7. Creen el gráfico de dispersión entre el recaudo total y el recaudo en el día del estreno como en la diapositiva 4. ¿Es consistente el gráfico con el valor de la correlación?
...
$$Promedio_i=\beta_0 + \beta_1H_i + \beta_2Saber11_i+\beta_3PCT_i+e_i$$
...
y &= \color{#0074D9}{\text{Modelo}} + \color{#FF851B}{\text{Error}} \\
  &= \color{#0074D9}{f(x)} + \color{#FF851B}{e}
\end{aligned}
$$
:::
...
- En *promedio*, una galleta adicional está asociado a un aumento en la felicidad de 0.16 unidades
...
- El intervalo de confianza al 95% no incluye el cero
...
## ¿Cómo visualizar la relación $y=f(x_1,x_2)+\varepsilon$? {.medium}
...
## ¿Cómo visualizar la "recta" en una regresión múltiple? {.medium}
...
## Galletas, Felicidad, Estudiantes y Profesores {.medium}
...
cookies_data2 |> 
  knitr::kable(format = "html") |> 
  kableExtra::kable_styling(font_size = 25) # Adjust the font size as needed
```
...
ggplot(cookies_data, aes(x = galletas, y = felicidad, color = grupo)) +
  geom_point(size = 3) +
...
:::{.fragment}
En promedio, los estudiantes tienen un nivel de felicidad $\hat{\beta_2}$ más alto que los profesores ($\hat{\beta_2}>0$ según la gráfica)
:::



## Otra manera de visualizar la variable categórica: {.medium}

El intercepto y $Estudiante$ controlan por las medias de ambos grupos:
...
cookies_data2 <- cookies_data2 |>
  mutate(y_dm = felicidad - mean_a * (estudiante == 1) - mean_b * (estudiante == 0))

ggplot(cookies_data, aes(x = galletas, y = felicidad, color = grupo, shape = grupo)) +
  geom_point(size = 4)  +
  geom_hline(yintercept = mean_a, color = "darkred", alpha = 1.5, size = 1) +
  geom_hline(yintercept = mean_b, color = "navy", alpha = 0.5, size = 1) +  
  labs(x = "Galletas consumidas", y = "Nivel de Felicidad", color = "Grupo", shape = "Grupo") +  # Make sure both labels match
  theme_minimal(base_size = 14) +
  coord_cartesian(xlim = c(0, 8), ylim = c(-2, 3)) +
  scale_color_manual(values = c("Profesores" = "navy", "Estudiantes" = "darkred")) +
  scale_shape_manual(values = c("Profesores" = 1, "Estudiantes" = 2)) +  # Keep both mapped to the same variable
  scale_x_continuous(breaks = 0:8) +
  theme(panel.grid.minor = element_blank())
...
Luego de remover las medias de ambos grupos:
...
ggplot(cookies_data2 |> 
       ...
       aes(x = galletas, y = y_dm, color = estudiante, shape=estudiante)) +
  geom_point(size = 4) +
  labs(x = "Galletas consumidas", y = "Nivel de Felicidad", color = "Grupo", shape = "Grupo") +  # Make sure both labels match
  theme_minimal(base_size = 14) +
  coord_cartesian(xlim = c(0, 8), ylim = c(-2, 3)) +
...
$\hat{\beta_1}$ estima la relación de interés luego de controlar por $Estudiante$:
...
$$e=y - \hat{y}= \text{US Gross} - \widehat{\text{US Gross}}$$
...
## Volvamos a Anscombe... {.medium}

Si estimamos $\hat{y}=\hat{\beta_0} + \hat{\beta_1} \times x$
...
ggplot(anscombe_m, aes(x, y)) +  
geom_text(aes(x = 15, y = 5, label=set), family = "Alfa Slab One", size=36, color = "gray95") + 
geom_smooth(method="lm", fill=NA, fullrange=TRUE, color = "darkred", linewidth=0.5) + 
geom_point(size=3.5, color="black", fill="red", alpha=0.8, shape=21) +
facet_wrap(~set, ncol=2, scales = "free") +
scale_x_continuous(limits = c(0,20)) +
scale_y_continuous(limits = c(0,15)) +
...
En cada regresión, $\hat{\beta_1}=0.5$. [Graficar los datos es crucial!]{.hl .hl-dred}
