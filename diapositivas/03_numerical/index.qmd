...
6. [Medidas de dispersión](#dispersion)

7. [Medidas de relación entre dos variables](#bivariado)
...
::: {.aside}
Para entender por qué el uso de "`/`" pueden leer [Forward Slash in R](https://www.geeksforgeeks.org/how-to-convert-backslash-to-forward-slash-in-r/){target="_blank"}
:::
...
1. Usando la plantilla con la que ya hemos trabajado anteriormente, establezcan el directorio de trabajo (la carpeta donde guardaron los datos de airbnb) como se muestra abajo. En esta sesión sólo usaremos el paquete `tidyverse` así que asegúrense de cargarlo al inicio.
...
2. Importen los datos `airbnb_ny_2019` usando la función `read.csv()` del paquete `tidyverse`:
...
- format $\rightarrow$ [categórica]{.marker-hl}

- number $\rightarrow$ [categórica]{.marker-hl}
:::
...
Regla general: [cuantas más observaciones haya, más barras se deben usar]{.hl .hl-dred}


## El gráfico de densidad  {.medium}
...
## La gráfica de densidad en R {.medium}
...
El [sesgo]{.marker-hl} mide qué tan asimétricos están distribuidos los datos
...
La [moda]{.marker-hl} es el dato que más se repite en la distribución
...
1. Calculen la media y la mediana de la variable `price` para cada `neighbourhood_group`.
...
- [Rango intercuartílico (RIC)]{.hl .hl-blue} = $Q_3 − Q_1$  

. . .
  
- [Resumen de cinco números]{.hl .hl-blue}: 
...
El [RIC]{.hl .hl-blue} es una medida de la variabilidad de los datos. El RIC tiende a ser mayor si la variación de los datos también es mayor.
...
Cada uno proporciona información diferente; por ejemplo, mediana vs moda 
...
1. Grafiquen los diagramas de caja de la variable `price` para cada `neighbourhood_group`.

2. Describan con 3 aspectos las 5 gráficas. Dos ejemplos: todos los barrios tienen una presencia fuerte de outliers y Manhattan tiene la mayor mediana.

3. Usando la función `IQR()`, calculen el rango intercuartílico de la variable `price` para cada barrio.
...
abline(v = 15.5, col = "purple", lty = 2, lwd = 2.2) # lty=2 for dashed line, lwd=2 for line width
...
- ¿Qué tan representativa es la media en cada uno de los gráficos?
...
1. Completen el siguiente código que crea la variable `within_1SD` que es igual a TRUE si el precio se encuentra dentro de una desviación estándar de la media, y FALSE en caso contrario.
...
_______ <- newdata |>
  mutate(within_1SD = price >= mean(_____, na.rm = TRUE) - sd(price, na.rm = TRUE) &
         price <= mean(price, na.rm = TRUE) + sd(_____, na.rm = TRUE))
```

2. Usando la función `View()` revisen la nueva variable en los datos.

3. Calculen la media de `within_1SD`. Recuerden que el promedio de una variable que toma valores 0 o 1 corresponde a la proporción de observaciones que cumplen la condición. ¿La proporción obtenida se aproxima al 68%?



# Medidas de relación entre dos variables {#bivariado}
...
La opción `facet_wrap()` separa el gráfico por grupos. Este [link](https://ggplot2.tidyverse.org/reference/facet_wrap.html) explica cómo usarla y da ejemplos.
...
## Valores y gráficos de la correlación {.medium}
